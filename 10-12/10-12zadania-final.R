...
# zorując się na Zadaniu 7 opracuj i zaimplementuj system rekomendacji filmów
...
# Big Data i uczenie maszynowe - Zadanie 11: 
# zorując się na Zadaniu 8 opracuj i zaimplementuj system rekomendacji filmów 
# w oparciu o filtrowanie kolaboratywne oparte na przedmiotach 
# (item-based collaborative filtering).

#### Data preparation

# Remove timestamp from rating column (causing trouble probably due to repetitions)
ratings <- ratings[,-4]

#### Data processing for item-based filtering
...
movie_user_item[1:4,1:4]

# Calculate the degree of sparsity (having many NAs)
sum(is.na(movie_user_item)) / ( ncol(movie_user_item) * nrow(movie_user_item) )
...
sample_movie <- sample(colnames(movie_user_item), 1) # get a random movie
recom_cf_item = item_recommendation(sample_movie)
recom_cf_item
...
visualizar_recomendacion(recom_cf_item[!is.na(recom_cf_item$movie_title),],
                         "Movie", "Year")
